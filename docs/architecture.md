...
- `scripts/pipeline.py` — **Pipeline CLI (Parts 15–19):** the bridge between the TS
  ...
  `fidelity_losses`, `metadata`); `render` (Part 19) has three modes — `--html`
  renders a generated standalone HTML (the shot), `--ir --layout` builds the
  intended VStyles via `web_common.reference_styles_from_plan` (the same
...
- `runtime/src/core/backend_codegen.ts` — **Runtime bridge (Parts 15–19):**
  ...
  real stage handlers (generate prefers the staged front-half artifacts, with a
  legacy `--file` fallback), registered by `figmaforge run` (eight real stages)
  and reused by `figmaforge demo`.
