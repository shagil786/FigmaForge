...
- `plugin/figmaforge/` — The primary source code and content:
  - `core/` (Python): The full Figma-to-Code pipeline:
    ...
    - **Pipeline CLI (Parts 15–19):** `scripts/pipeline.py` — `ingest`, `normalize`, `resolve`, `layout`, `assets`, `generate`, and `render` subcommands (one-JSON-line contract, exit codes 2/3/4); `generate` supports staged mode (`--ir/--layout/[--resolution]`, byte-identical to `--file` recompute); `assets` downloads + content-addresses IR image/SVG refs (SVG-validated, deterministic manifest); `render` has three modes (`--html` shot, `--ir --layout` reference baseline via the shared web lowering, `--baselines` live Figma download); consumed by the TS runtime
...
- `runtime/` (Part 9 — TypeScript Orchestration Runtime):
  ...
  - `tests/` (3 files, 141 tests): Comprehensive test suite with custom test framework (incl. backend-codegen stage, demo, six-stage front-half, and eight-stage render/compare tests)
