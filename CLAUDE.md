...
- `plugin/figmaforge/` — The primary source code and content:
  ...
  - `tests/` (39 test files, 461 tests): Unit, integration, snapshot, property-based, repair-loop, backend adapter (six real generators + 5 golden snapshots), capability-vs-output honesty audit, render-harness, pixel-diff, SSIM-gating, and baseline-refresh tests.
...
- All 461 Python tests across 39 test files must pass (`python3 -m unittest discover -s tests`); browser-render tests skip cleanly without chromium.
...
4. Verify all 461 tests pass and regenerate snapshots if output changed intentionally.
...
- Full test suite passes: `python3 -m unittest discover -s tests` (461 tests, 39 files).
