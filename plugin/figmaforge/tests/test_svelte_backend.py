...
class TestSvelteBackend(unittest.TestCase):
    ...
    def test_image_fill_resolved_emits_scoped_css(self):
        ...
        component = [f for f in output.files if f.path.endswith(".svelte")][0]
        self.assertIn("background-image: url(assets/photo.png)", component.content)
        self.assertIn("background-size: cover", component.content)
        self.assertIn("background-position: center", component.content)
        self.assertNotIn("fills_image approximated", component.content)
